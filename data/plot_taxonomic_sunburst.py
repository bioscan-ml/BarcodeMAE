...
def make_sunburst(df, levels, out_path, color_rank, cat_color):
    ...
    fig.update_layout(
        xaxis={"visible": False},
        yaxis={"visible": False},
        legend={"title": color_rank.capitalize(), "x": 1.0, "y": 0.5, "xanchor": "left", "yanchor": "middle"},
        margin={"t": 10, "l": 10, "r": 180, "b": 10},
    )
    # PNG (not PDF): the reference-set sunbursts have tens of thousands of
    # genus-level wedges, and as vector PDFs that made the figures extremely
    # slow to render/compile. A rasterized PNG at scale=1.25 (1500x1250px)
    # is plenty for print at the sub-figure widths used in the appendix.
    fig.write_image(out_path, width=1200, height=1000, scale=1.25)
    print(
        f"  Wrote {out_path}  ({len(df)} specimens, {len(present_levels)} ranks: {present_levels}, "
...
def run_bioscan5m(data_dir, out_dir):
    ...
    make_sunburst(ref_df, LEVELS, os.path.join(out_dir, "taxdist_bioscan5m_reference.png"), color_rank, cat_color)
    make_sunburst(query_df, LEVELS, os.path.join(out_dir, "taxdist_bioscan5m_query.png"), color_rank, cat_color)
...
def filter_genus_level_queries(train_df, test_df):
    ...
    is_novel_species = pd.Series([k not in train_species for k in species_keys], index=clean_known.index)
...
def run_its5m(data_dir, out_dir, query_fasta, query_name):
    ...
    make_sunburst(
        taxonomy_columns(train_df), LEVELS, os.path.join(out_dir, "taxdist_its5m_reference.png"), color_rank, cat_color
    )

    raw_query_df = load_its5m_full(os.path.join(data_dir, query_fasta))
    query_df = filter_genus_level_queries(train_df, raw_query_df)
    print(
        f"  {query_name}: {len(raw_query_df)} raw specimens -> {len(query_df)} leakage-free "
        f"genus-level query specimens"
    )
    make_sunburst(
        taxonomy_columns(query_df),
        LEVELS,
        os.path.join(out_dir, f"taxdist_its5m_query_{query_name}.png"),
        color_rank,
        cat_color,
    )
