#!/bin/bash
# ============================================================================
# Fungi ITS Final Experiments — Pretraining (15 epochs) + Finetuning (12 epochs)
#
# 10 array tasks (0-9), mirroring the BIOSCAN-5M grid:
#
...
#
# Finetuning representation types per model:
#   no-CLS models → tokens
#   CLS models    → tokens | cls | tokens_with_cls
#
# Checkpoints saved under: main_checkpoints_final/ITS-5M/
# ============================================================================
#SBATCH --job-name=final_fungi_its
...
if [ "$ARCH" = "maelm" ]; then
    RUN_NAME="final_its_k${K_MER}_${N_LAYERS}L${N_HEADS}H_${N_DEC_LAYERS}DL${N_DEC_HEADS}DH_${ARCH}_${CLS_LABEL}"
else
    RUN_NAME="final_its_k${K_MER}_${N_LAYERS}L${N_HEADS}H_${ARCH}_${CLS_LABEL}"
fi
...
# ── Select checkpoint for finetuning ─────────────────────────────────────────
...
if [ ! -f "${FT_CKPT}" ]; then
    echo "ERROR: Finetuning checkpoint not found at ${FT_CKPT}"
    exit 1
fi

# ── Representation types for finetuning ──────────────────────────────────────
# no-CLS models: only mean-pool sequence tokens (no CLS to use)
# CLS models: evaluate all three pooling strategies
if [ "$HAS_CLS_VAL" = "no" ]; then
    FT_REP_TYPES=("tokens")
else
    FT_REP_TYPES=("tokens" "cls" "tokens_with_cls")
fi

OVERALL_EXIT=0

# ── Finetuning loop ───────────────────────────────────────────────────────────
echo "=== FINETUNING (taxa=${FT_TAXA}, lr=${FT_LR}, epochs=${FT_EPOCHS}) ==="
for REPR in "${FT_REP_TYPES[@]}"; do
    FT_RUN="${RUN_NAME}_ft_${FT_TAXA}_${REPR}_ep${FT_EPOCHS}"
    FT_CKPT_OUT="${CKPT_BASE}/finetune/${FT_RUN}.pt"
    FT_DONE="${CKPT_BASE}/finetune/.done_${FT_TAXA}_${REPR}"

    if [ -f "${FT_DONE}" ]; then
        echo "Finetuning (repr=${REPR}) already done — skipping."
        continue
    fi

    echo "--- Finetuning: repr=${REPR} ---"
    torchrun --standalone --nproc_per_node=1 barcodebert/finetuning.py \
        --run-name              "${FT_RUN}"     \
        --dataset               "${DATASET}"    \
        --data-dir              "${DATA_DIR}"   \
        --pretrained-checkpoint "${FT_CKPT}"    \
        --checkpoint            "${FT_CKPT_OUT}" \
        --taxonomic-level       "${FT_TAXA}"    \
    ...
    FT_EXIT=$?
    if [ ${FT_EXIT} -eq 0 ]; then
        touch "${FT_DONE}"
    else
        echo "ERROR: Finetuning failed for repr=${REPR} (exit ${FT_EXIT})"
        OVERALL_EXIT=${FT_EXIT}
    fi
done
