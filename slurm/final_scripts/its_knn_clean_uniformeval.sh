#!/bin/bash
# ============================================================================
# ITS-5M Leakage-Free KNN Evaluation — UNIFORM (plain majority) vote variant,
...
# uniform fresh here, on the same grid/checkpoints/k-range as
# its_knn_clean_softmaxeval.sh, gives one clean, directly-comparable,
# same-environment baseline instead of stitching together old sources.
#
# Identical grid-building logic to its_knn_clean_softmaxeval.sh (same
# checkpoints, same representations, same 35-task grid), just omits
# --knn-weights (uniform is knn_vote()'s default) and --temperature (unused
# for uniform). Results auto-route to KNN_ITS_CLEAN_RESULTS.txt unchanged
# (no "uniform_" prefix -- that's the default/unprefixed file per
# knn_results_path() in evaluation.py).
#
# Covers 35 indices (0-34): the 10 main checkpoints (fungi_its_final.sh --
# nocls=1 repr, cls_*=3 reprs each -> 2x1 + 8x3 = 26) plus the 2
# mining-ablation checkpoints (ablation_triplet_mining.sh, cls_triplet
# random mining -- 3 reprs each -> 6), plus the random-init baseline
# (transformer only, 3 configs: nocls/tokens, cls/cls, cls/tokens_with_cls).
#
...
#
# Submit:  sbatch its_knn_clean_uniformeval.sh
# ============================================================================
#SBATCH --job-name=its_knn_clean_uniform
...
# ── Build the (ckpt|arch|label|repr) grid — identical to its_knn_clean_softmaxeval.sh ──
...
if [ "${CKPT}" = "RANDOM" ]; then
    RUN_NAME="uniformknn_random_k${K_MER}_${N_LAYERS}L${N_HEADS}H_${ARCH}_${LABEL}_${REPR}"
    USE_CLS_ARGS=()
    ...
    RUN_NAME="uniformknn_${ARCH}_${LABEL}_${REPR}"
