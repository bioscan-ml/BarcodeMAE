#!/bin/bash
# ============================================================================
# Fungi ITS Final Experiments — Pretraining (15 ep) + Finetuning (12 ep)
#
# 10 array tasks (0-9):
...
#
# Finetuning repr types: no-CLS → tokens | CLS → tokens, cls, tokens_with_cls
# Each repr gets its own subdir → separate test_results_species.json (no overwrite)
# All 3 ITS test sets evaluated automatically by finetuning.py
# Wandb project:  barcodemae_cls  (offline → wandb_final/)
# Checkpoints:    main_checkpoints_final/ITS-5M/
# ============================================================================
...
# ── Finetuning ────────────────────────────────────────────────────────────────
# Each repr gets its own subdir → test_results_species.json never overwritten
...
for REPR in "${FT_REP_TYPES[@]}"; do
    ...
    FT_CKPT_OUT="${FT_REPR_DIR}/${FT_RUN}.pt"
    FT_DONE="${FT_REPR_DIR}/.done"

    mkdir -p "${FT_REPR_DIR}"
    [ -f "${FT_DONE}" ] && echo "Finetuning (repr=${REPR}) already done — skipping." && continue

    echo "--- Finetuning: repr=${REPR} → ${FT_REPR_DIR}/test_results_${FT_TAXA}.json ---"
    ...
    FT_EXIT=$?
    [ ${FT_EXIT} -eq 0 ] && touch "${FT_DONE}" || { echo "ERROR: finetune failed repr=${REPR}"; OVERALL_EXIT=${FT_EXIT}; }
done
