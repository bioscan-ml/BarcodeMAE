...
# BarcodeMAE+ configuration (encoder-decoder, +CLS+Binary, CLS
# representation -- the config reported in tab:its_external and
# fig:main_kcurve). Fills a gap: no existing script ran a real
# --temperature-sweep for our own model on ITS-5M before (only a
# fixed-temperature-0.07 variant existed, its_knn_clean_softmaxeval.sh),
# so the exact source of the previously-reported "76.43 (k=25)" / "65.21
# (k=50)" Softmax KNN values was never independently verified.
#
# REQUIRES its_export_tasks.sh to have been run first (produces
# data/ITS-5M/tasks/test{1,2,3}_tasks.csv).
#
# Results: results_final/KNN_ITS_own_model_sweep_RESULTS.txt
#
# Submit: sbatch slurm/final_scripts/its5m_own_model_sweep.sh
# ============================================================================
#SBATCH --job-name=its5m_own_model_sweep
...
python barcodebert/knn_its_clean.py \
    ...
    --knn-weights uniform \
    --run-name knn_own_model_uniform \
    --results-file results_final/KNN_ITS_own_model_sweep_RESULTS.txt \
...
python barcodebert/knn_its_clean.py \
    ...
    --run-name knn_own_model_softmax_sweep \
    --results-file results_final/KNN_ITS_own_model_sweep_RESULTS.txt \
    --log-wandb --wandb-project barcodemae_cls
