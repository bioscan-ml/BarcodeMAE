#!/bin/bash
# ============================================================================
# ITS-5M Leakage-Free KNN Evaluation — SOFTMAX (temperature-weighted) vote
# variant, matching DINOv2's kNN eval weighting (softmax(similarity/T)).
#
# Identical grid-building logic to its_knn_clean_eval.sh / its_knn_clean_softeval.sh
# (same checkpoints, same representations), just uses --knn-weights softmax
# --temperature 0.07 instead of uniform/distance. Results auto-route to a
# separate file (KNN_softmax_ITS_CLEAN_RESULTS.txt) via knn_results_path()
# in evaluation.py, so this never collides with the uniform-vote or
...
#
# Results: results_final/KNN_softmax_ITS_CLEAN_RESULTS.txt
#
# Submit:  sbatch its_knn_clean_softmaxeval.sh
# ============================================================================
#SBATCH --job-name=its_knn_clean_softmax
...
if [ "${CKPT}" = "RANDOM" ]; then
    RUN_NAME="softmaxknn_random_k${K_MER}_${N_LAYERS}L${N_HEADS}H_${ARCH}_${LABEL}_${REPR}"
    USE_CLS_ARGS=()
    ...
    RUN_NAME="softmaxknn_${ARCH}_${LABEL}_${REPR}"
