...
# CE). Tasks 0-3 (Binary) were already run in an earlier submission (before
...
# submit the 8 NEW (Triplet + CE) tasks, use: sbatch --array=4-11 <this
# script>.
#
...
# even if this chunk is killed by SLURM mid-epoch on walltime. This relies on
...
# same --checkpoint path across chunks is all that's needed -- no new resume
# flag required.
#
...
# decision (tasks 0-3/Binary, trained before this ablation existed) is now
...
# (at the new T=0.02) is re-run in that case.
OVERALL_EXIT=0
