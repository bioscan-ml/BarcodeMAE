#!/bin/bash
# ============================================================================
# SAME 12 tasks as bioscan5m_aux_weight_ablation.sh, but for a cluster whose
# project storage is only reachable via the ~/projects/def-lila-ab/ symlink
# form (no /project/6045013/ numeric mount there). Directory convention here
# matches its_aux_weight_ablation_home.sh's DATA_DIR/DATA_ROOT/CKPT_BASE/
# WANDB_DIR pattern exactly. Account, modules, and venv path are otherwise
# identical to bioscan5m_aux_weight_ablation.sh.
#
...
# Each task now requests only 3h instead of one giant ~19-20h request, since
# even a 6h request sat with no resource allocation. At startup, EACH job --
...
#
# Submit (all 12): sbatch slurm/final_scripts/bioscan5m_aux_weight_ablation_home.sh
# Submit (new 8 only): sbatch --array=4-11 slurm/final_scripts/bioscan5m_aux_weight_ablation_home.sh
# ============================================================================
#SBATCH --job-name=bioscan5m_aux_weight_ablation_home
...
# Checkpoint/wandb root derived from DATA_DIR (mirrors its_aux_weight_ablation_home.sh):
