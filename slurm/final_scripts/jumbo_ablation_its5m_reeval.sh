...
#
# Unlike the original script (one array task per checkpoint, looping over
# all 3 reprs x 2 voting modes sequentially), this splits every
# (checkpoint, repr, voting) combination into its OWN array task, so a
# failure/OOM in one no longer burns the time budget needed for the rest --
# same fix used for the ITS family-ablation eval split earlier.
#
...
#
# Submit everything:      sbatch slurm/final_scripts/jumbo_ablation_its5m_reeval.sh
# Submit specific tasks:  sbatch --array=0,1 slurm/final_scripts/jumbo_ablation_its5m_reeval.sh
# ============================================================================
