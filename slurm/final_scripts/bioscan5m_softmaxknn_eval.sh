#!/bin/bash
# ============================================================================
# BIOSCAN-5M SOFTMAX (temperature-weighted) KNN eval, matching DINOv2's kNN
# eval weighting (softmax(similarity/T)). Reproduces the full main sweep +
# mining ablation + random-init baseline, mirroring bioscan5m_softknn_eval.sh
# (the distance-weighted version) but with --knn-weights softmax
# --temperature 0.07.
#
# Unlike bioscan5m_softknn_eval.sh, the random-init baseline IS included
# here: random_knn.py was updated (this session) to support
# --knn-weights/--temperature, so it's no longer excluded.
#
...
#     only (nocls/tokens, cls/cls, cls/tokens_with_cls) -- matches the
#     ITS-5M random-baseline scope exactly, no maelm variant.
#
# Output: results_final/KNN_softmax_RESULTS_final.txt (main+mining, via
# knn_probing.py) and results_final/softmax_RANDOM_KNN_RESULTS.txt (random
# baseline, via random_knn.py) -- both auto-routed by knn_results_path()
# in evaluation.py.
#
# Submit:  sbatch slurm/final_scripts/bioscan5m_softmaxknn_eval.sh
# ============================================================================
#SBATCH --job-name=bioscan_softmaxknn
...
# Matches its_knn_clean_softmaxeval.sh's random-baseline scope exactly.
...
if [ "${CKPT}" = "RANDOM" ]; then
    RUN_NAME="softmaxknn_${TAG}_${REPR}"
    USE_CLS_ARGS=()
