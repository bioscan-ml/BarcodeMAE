...
# NEW: Import mycoai utils for LEVELS
try:
    from mycoai import utils as mycoai_utils

    LEVELS = mycoai_utils.LEVELS
except ImportError:
    # Fallback if mycoai is not installed
...
class ClassificationModel(nn.Module):
    """Original single-level classifier."""
...
class InferSumClassificationModel(nn.Module):
    """
    Hierarchical classifier using InferSum approach.
    Predicts at base level (e.g., species), infers parent probabilities.
    Supports hierarchical loss when labels at finer levels are missing.
    """

    def __init__(self, base_model, classes, tax_encoder, max_level='species'):
        """
        Parameters
        ----------
        base_model : transformer model
            Pretrained encoder model
        classes : list of 6 ints
            Number of classes at each taxonomic level [phylum, class, order, family, genus, species]
        tax_encoder : TaxonEncoder
            From mycoai, contains inference matrices for parent probability inference
        max_level : str
            Taxonomic level to predict at (default 'species')
        """
        super(InferSumClassificationModel, self).__init__()
        self.base_model = base_model
        self.hidden_size = self.base_model.config.hidden_size
        self.tax_encoder = tax_encoder
        self.max_level = LEVELS.index(max_level)
        self.classes = classes

        # Single linear layer for base level prediction
        self.classifier = nn.Linear(self.hidden_size, classes[self.max_level])
        self.softmax = nn.Softmax(dim=1)

        # For compatibility with existing code
        self.num_labels = classes[self.max_level]

    def forward(self, input_ids=None, mask=None, labels=None):
        """
        Forward pass with hierarchical loss support.

        Parameters
        ----------
        labels : tensor
            If shape (batch,) - single level labels (original behavior)
            If shape (batch, 6) - hierarchical labels with -1 for missing
        """
        # Get embeddings
        outputs = self.base_model(input_ids=input_ids, attention_mask=mask)
        embeddings = outputs.hidden_states[-1]
        ...
        if labels is not None:
            if labels.dim() == 1:
                ...
                # Hierarchical labels (batch, 6) - use hierarchical loss
                all_level_probs = self._get_all_level_probs(logits)
                loss = self._hierarchical_loss(all_level_probs, logits, labels)

        return TokenClassifierOutput(loss=loss, logits=logits, hidden_states=outputs.hidden_states)

    def _get_all_level_probs(self, base_logits):
        """Get probability predictions at all 6 levels from base level logits."""
        device = base_logits.device
        batch_size = base_logits.shape[0]

        # Initialize output with zeros for levels above max_level
        output = [torch.zeros((batch_size, self.classes[lvl]), device=device)
                  for lvl in range(5, self.max_level, -1)]

        # Base level probabilities
        base_probs = self.softmax(base_logits)
        output.insert(0, base_probs)

        # Infer parent probabilities
        for i in range(self.max_level - 1, -1, -1):
            parent_probs = self.tax_encoder.infer_parent_probs(output[0], i)
            output.insert(0, parent_probs)

        return output  # [phylum, class, order, family, genus, species]

    def _hierarchical_loss(self, predictions, base_logits, labels):
        """
        Compute loss at the finest level that has a valid label for each sample.

        Parameters
        ----------
        predictions : list of 6 tensors
            Probability distributions at each level
        base_logits : tensor
            Raw logits at base level (for proper CE loss)
        labels : tensor of shape (batch_size, 6)
            Labels at each level, -1 for missing
        ...
        for sample_idx in range(batch_size):
            # Find the finest level with a valid label (prefer species > genus > ... > phylum)
            for lvl in range(5, -1, -1):  # species=5 down to phylum=0
                if labels[sample_idx, lvl] != -1:
                    target = labels[sample_idx, lvl].unsqueeze(0)

                    if lvl == self.max_level:
                        # Use logits for base level (proper cross entropy with logits)
                        sample_logits = base_logits[sample_idx:sample_idx + 1]
                        sample_loss = F.cross_entropy(sample_logits, target)
                    else:
                        # Use log probabilities for inferred (parent) levels
                        # Add small epsilon for numerical stability
                        probs = predictions[lvl][sample_idx:sample_idx + 1]
                        log_probs = torch.log(probs + 1e-10)
                        sample_loss = F.nll_loss(log_probs, target)

                    total_loss = total_loss + sample_loss
                    num_samples += 1
                    break  # Only use finest available level for this sample
    ...
    def predict_all_levels(self, input_ids=None, mask=None):
        """
        Get predictions at all taxonomic levels (for inference/evaluation).

        Returns
        -------
        output : list of 6 tensors
            Probability distributions at each level [phylum, class, order, family, genus, species]
        """
        outputs = self.base_model(input_ids=input_ids, attention_mask=mask)
        embeddings = outputs.hidden_states[-1]
        GAP_embeddings = embeddings.mean(1)
        logits = self.classifier(GAP_embeddings)
        return self._get_all_level_probs(logits)
...
def run(config):
    ...
    keys_to_reuse = [
        ...
        "tokenize_n_nucleotide", "predict_n_nucleotide", "pretrain_levenshtein",
        "levenshtein_vectorized", "n_layers", "n_heads", "dataset_name",
    ]
    ...
    if config.dataset_name == "ITS-5M":
        dataset_train = DNADataset(
            ...
            **dataset_args,
            ...
            use_hierarchical=config.use_infersum,  # NEW: Enable hierarchical labels for InferSum
        )

        # Share tax_encoder across datasets for InferSum
        shared_tax_encoder = dataset_train.tax_encoder if config.use_infersum else None
        ...
        dataset_val = DNADataset(
            ...
            **dataset_args,
            taxonomic_level=config.taxonomic_level,
            label2id=None,
            tax_encoder=shared_tax_encoder,
            use_hierarchical=config.use_infersum,  # NEW
        )
    ...
    # MODEL ===================================================================
    if config.use_infersum and config.dataset_name == "ITS-5M":
        # Use InferSum hierarchical classifier
        if dataset_train.tax_encoder is None:
            raise ValueError("tax_encoder is required for InferSum but was not created.")
        if dataset_train.classes_per_level is None:
            raise ValueError("classes_per_level is required for InferSum but was not computed.")

        model = InferSumClassificationModel(
            pre_model,
            classes=dataset_train.classes_per_level,
            tax_encoder=dataset_train.tax_encoder,
            max_level=config.taxonomic_level
        )
        print(f"Using InferSum classifier with hierarchical loss")
        print(f"  Classes per level: {dataset_train.classes_per_level}")
        print(f"  Predicting at level: {config.taxonomic_level} (index {model.max_level})")
    else:
        # Use original single-level classifier
        model = ClassificationModel(pre_model, dataset_train.num_labels)
        print(f"Using standard classifier with {dataset_train.num_labels} labels")
    ...
    if config.log_wandb and config.global_rank == 0:
        ...
        EXCLUDED_WANDB_CONFIG_KEYS = [
            "log_wandb", "wandb_entity", "wandb_project", "global_rank",
            "local_rank", "run_name", "run_id", "model_output_dir",
        ]
        job_type = "linearprobe" if config.freeze_encoder else "finetune"
        if config.use_infersum:
            job_type = f"{job_type}_infersum"
        wandb.init(
            name=wandb_run_name,
            id=config.run_id,
            resume="allow",
    ...
    if not config.checkpoint_path and config.models_dir:
        ...
        config.checkpoint_path = os.path.join(config.model_output_dir,
                                              f"checkpoint_finetune_{config.taxonomic_level}.pt")
        if config.log_wandb and config.global_rank == 0:
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        if config.model_output_dir and (not config.distributed or config.global_rank == 0):
            ...
            if config.use_infersum:
                model_deref = model.module if config.distributed else model
                save_dict["classes_per_level"] = model_deref.classes
                save_dict["use_infersum"] = True
    ...
    if config.dataset_name == "ITS-5M":
        dl_train_eval_kwargs = copy.deepcopy(dl_test_kwargs1)
    else:
        dl_train_eval_kwargs = copy.deepcopy(dl_test_kwargs)

    if config.distributed:
        dl_train_eval_kwargs["sampler"] = DistributedSampler(
            dataset_train_eval, shuffle=False, drop_last=False,
        )
...
def train_one_epoch(
    ...
    for batch_idx, (sequences, y_true, att_mask) in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            if y_true.dim() > 1:
                # Hierarchical labels - compute accuracy at base level (species)
                # Get model's max_level
                model_deref = model.module if config.distributed else model
                max_lvl = model_deref.max_level
                y_true_single = y_true[:, max_lvl]
                # Only count samples with valid labels at base level
                valid_mask = y_true_single != -1
                if valid_mask.sum() > 0:
                    is_correct = y_pred[valid_mask] == y_true_single[valid_mask]
...
def get_parser():
    ...
    # NEW: Add --use-infersum argument
    group.add_argument(
        "--use-infersum",
        "--use_infersum",
        dest="use_infersum",
        action="store_true",
        help="Use InferSum hierarchical classifier. Enables training on samples "
             "without species labels by computing loss at the finest available level.",
    )
